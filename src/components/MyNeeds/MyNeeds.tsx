...
const SortableSelectedNeed = SortableElement<{
  need: Need;
  toggleNeed: (need: Need) => void;
}>(({ need, toggleNeed }: { need: Need; toggleNeed: (need: Need) => void }) => {
  return (
    <li className={styles.selectedNeed}>
      <h4>
        <span className={styles.removeButton} onClick={() => toggleNeed(need)}>
          X
        </span>
...
const SortableSelectedNeeds = SortableContainer<{
  selectedNeeds: Need[];
  toggleNeed: (need: Need) => void;
}>(
  ({
...
export const MyNeeds = () => {
  const [needs, setNeeds] = React.useState<NeedsByCategory[]>([]);
  ...
  const toggleNeed = (need: Need) => {
    if (selectedNeeds.includes(need.id)) {
      const newList = selectedNeeds.filter(
        (selectedNeed) => selectedNeed !== need.id,
      );
      setSelectedNeeds(newList);
      saveToLocalStorage(newList);
    } else {
      const newList = [...selectedNeeds, need.id];
      setSelectedNeeds(newList);
      saveToLocalStorage(newList);
    }
  };

  const listOfSelectedNeeds = selectedNeeds
    .map((needId) => {
      const need = needs.find((need) =>
        need.needs.find((need) => need.id === needId),
      );
      if (need) {
  ...
  return (
    <div className={styles.MyNeeds}>
      ...
      <div className={styles.SelectedNeeds}>
        ...
        <SortableSelectedNeeds
          selectedNeeds={listOfSelectedNeeds}
          toggleNeed={toggleNeed}
          onSortEnd={onSortEnd}
          helperClass={styles.floatingSortableListElement}
        />
      </div>
    </div>
  );
